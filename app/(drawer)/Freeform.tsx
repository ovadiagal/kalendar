...
const Freeform = () => {
  ...
  const data = [
    {
      key: 'What other information would you like to add about your work preferences?',
      description:
    ...
    {
      key: 'What other information would you like to add about your break preferences?',
      description:
      ...
      placeholder: 'e.g., working out helps me get in the zone',
      value: preferences.breakPreferences,
      inputKey: 'breakPreferences',
    },
    {
      key: 'Is there anything else that might be helpful to know?',
      description:
  ...
  return (
    <View className="flex-1 bg-gray-100">
      <Text className="mb-5 pl-5 pr-5 pt-10 text-3xl font-bold">
        Feel free to customize your schedule
      </Text>
      <FlatList
        data={data}
        renderItem={({ item }) => (
          <View className="flex-1 rounded-lg bg-white p-5" style={{ width }}>
            <Text className="mb-6 text-lg font-semibold">{item.key}</Text>
            <TextInput
              value={item.value}
