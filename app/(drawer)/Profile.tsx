...
const Profile = () => {
  ...
  const fetchPreferences = async () => {
    try {
      ...
      const structuredTimestamp = workData?.[0]?.updated_at || breakData?.[0]?.updated_at;
      const freeformTimestamp = freeformData?.[0]?.updated_at;
  
      if (structuredTimestamp && freeformTimestamp) {
        const structuredDate = new Date(structuredTimestamp);
        const freeformDate = new Date(freeformTimestamp);
  
        if (freeformDate > structuredDate) {
          setIsFreeform(true);
      ...
      } else if (freeformTimestamp) {
        setIsFreeform(true);
        setFreeformPreferences(JSON.parse(freeformData[0].preference_modifications));
      } else if (structuredTimestamp) {
        setIsFreeform(false);
        setWorkPreferences({
          days: workData[0].days.map(day => ({
            ...day,
          ...
          {isFreeform ? (
            <View>
              <View className="mb-6 rounded-lg bg-white p-4 shadow-sm">
                <Text className="mb-2 text-lg font-semibold">Work Preferences</Text>
                {isEditing ? (
                  <TextInput
                    value={freeformPreferences.workPreferences}
                    onChangeText={(text) => 
                      setFreeformPreferences(prev => ({...prev, workPreferences: text}))}
              ...
              <View className="mb-6 rounded-lg bg-white p-4 shadow-sm">
                <Text className="mb-2 text-lg font-semibold">Break Preferences</Text>
                {isEditing ? (
                  <TextInput
                    value={freeformPreferences.breakPreferences}
                    onChangeText={(text) => 
                      setFreeformPreferences(prev => ({...prev, breakPreferences: text}))}
                    className="rounded-lg border border-gray-200 p-2"
                    multiline
                  />
                ) : (
              ...
              <View className="rounded-lg bg-white p-4 shadow-sm">
                <Text className="mb-2 text-lg font-semibold">Additional Preferences</Text>
                {isEditing ? (
                  <TextInput
                    value={freeformPreferences.additionalPreferences}
                    onChangeText={(text) => 
                      setFreeformPreferences(prev => ({...prev, additionalPreferences: text}))}
                    className="rounded-lg border border-gray-200 p-2"
                    multiline
                  />
                ) : (
            ...
            <View>
              <View className="mb-6 rounded-lg bg-white p-4 shadow-sm">
                <Text className="mb-4 text-lg font-semibold">Work Schedule</Text>
              ...
              <View className="mb-6 rounded-lg bg-white p-4 shadow-sm">
                <Text className="mb-4 text-lg font-semibold">Break Preferences</Text>
